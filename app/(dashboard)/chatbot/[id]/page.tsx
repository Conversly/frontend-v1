...
import UpperHeader from '@/components/landing/navbar';
import { motion } from 'framer-motion';
import { Database, MessageSquare, Palette, Settings2, BarChart, Bot } from 'lucide-react';
import { useActiveTab, useSetActiveTab, TabValue } from '@/store/chatbot/ui';
...
export default function ChatbotCustomizationPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const chatbotResponse = await getChatbot(id?.toString() ?? '');
        setChatbotData({
          ...chatbotResponse,
          System_Prompt: chatbotResponse.systemPrompt,
        });
          ...
          <Tabs defaultValue={activeTab} onValueChange={(value) => setActiveTab(value as TabValue)} className="space-y-8">
            ...
            <div className="space-y-6">
              ...
              <TabsContent value="customize">
                <CustomizationTab chatbotId={id?.toString() ?? ''} systemPrompt={chatbotData.systemPrompt} />
              </TabsContent>

              <TabsContent value="system">
                <SystemPromptTab chatbotId={id?.toString() ?? ''} systemPrompt={chatbotData.systemPrompt} />
              </TabsContent>
